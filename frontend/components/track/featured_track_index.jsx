...
export default class FeaturedTrackIndex extends React.Component {
  ...
  toggleList(e) {
    ...
    let other = Array.from(element.classList).includes("featured")
      ? document.querySelector(".discussed")
  ...
  render() {
    let subheading =
      this.state.list === "featured"
        ? "Hear Soundloud's most popular tracks:"
        : "Hear Soundloud's most discussed tracks:";
    return (
      <div className="l-container l-content">
        <div>
          <div className="l-fluid-fixed">
            <div className="featured__title">
              <h2 className="featured selected" onClick={this.toggleList}>
                Featured Tracks
              </h2>
              <h2 className="discussed" onClick={this.toggleList}>
              ...
              {this.props.featuredTracks[0].map((track, key) => {
                return (
                  <FeaturedTrackItem
                    key={`featured-${key}`}
                    track={track}
                    artist={this.props.featuredTracks[1][track.artist_id]}
                  />
                );
              })}
            </ul>
